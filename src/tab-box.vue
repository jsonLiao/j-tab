<template>
  <div class="tabbar" :class="{'tabbar-fix-bottom': fixBottom}" ref="viewArea">
    <div class="tab-list" :style="style" ref="list">
      <slot></slot>
      <div class="tab-active-bar" v-if="!fixBottom" :style="activeBarStyle" ref="activeBar">
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      calcBarPosX() {
        ...
        this.activeBarWidth = Math.max(itemWidth * 0.9, 14)
...
<style lang="scss">
  @import 'src/assets/scss/variables/_variable';
  @function px($num) {
    @return px2rem($num);
  }
  .tabbar {
